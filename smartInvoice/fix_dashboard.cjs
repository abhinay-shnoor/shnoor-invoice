...
// Find the index of 'return (' right after shouldRenderAccessDenied
const marker = 'const shouldRenderAccessDenied = isAdminTab && !isAuthorized';
const markerIdx = text.indexOf(marker);

if (markerIdx !== -1) {
  // Find the exact return statement after the marker
  const returnIdx = text.indexOf('return (', markerIdx);
  if (returnIdx !== -1) {
    // Cut the text right before the return statement
    text = text.substring(0, returnIdx);
    
    // Add the new helper methods and the correct return block
    const additions = `
